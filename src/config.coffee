...
GM_config.init
	...
	events: open: (configDocument, configWindow) ->
		...
		DOLLAR('head').append($('<link rel=\'stylesheet\'>').attr('href', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css')).append($('<script>').attr('src', 'https://code.jquery.com/jquery-latest.js')).append $('<script>').attr('src', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js')
		...
		DOLLAR('body').append '<a id="empty-zotero-cache" class="btn btn-danger">Empty Zotero Cache</a>'
		DOLLAR('#empty-zotero-cache').on 'click tap', ->
			GM_xmlhttpRequest
				method: 'DELETE'
				url: ZOTERO_API + '/cache'
				onload: (response) ->
					console.warn "Emptied Zotero Cache", response
